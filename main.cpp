...
// Watches an RC522 reader on a background thread (SPI polling plus, on a
// successful tap, a blocking POST /api/ClassSessions/auto-checkin) so
// neither stalls the render loop. Edge-triggered on card presence: a card
// left sitting in the field only fires once, and tapping the same card
// again after removing it fires again. Taps that arrive faster than the
// main thread displays them queue up instead of overwriting each other, so
// a run of students tapping in quick succession each still get their own
// few seconds on screen.
class NfcCheckInWorker
{
public:
    NfcCheckInWorker(ApiClient& client, std::string spiDevice)
        : client_(client), spiDevice_(std::move(spiDevice)), thread_(&NfcCheckInWorker::Run, this)
    {
    }
...
private:
    ...
    {
        NfcReader reader;
        if (!reader.Open(spiDevice_))
        {
            printf("NfcCheckInWorker: NFC reader unavailable on %s; auto-checkin disabled.\n", spiDevice_.c_str());
            return;
        }
        ...
        while (!stop_.load())
        {
            ...
            bool present = reader.PollForUid(uid);
...
int main(int argc, char* argv[])
{
    ...
    // Auto-checkin (RC522 NFC tap) is only meaningful under endpoint (device)
    // auth: the server resolves both the student and the class session from
    // the card UID plus the endpoint's own room/key, so there's no session
    // to pick under user auth anyway. NfcCheckInWorker degrades gracefully
    // (logs once, stays idle) if no reader is wired up on this machine.
    std::unique_ptr<NfcCheckInWorker> nfcWorker;
    if (apiClient.IsEndpointAuth())
    {
        std::string nfcSpiDevice = dotenv::Get("ATTENDANCE_NFC_SPI_DEVICE", "/dev/spidev0.0");
        nfcWorker.reset(new NfcCheckInWorker(apiClient, nfcSpiDevice));
    }
